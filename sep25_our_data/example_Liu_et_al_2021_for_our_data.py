# Example script for hierarchical cluster analysis (HCA) of groundwater chemistry data
#   and plotting Piper, Gibbs, and Gaillardet diagrams using WQChartPy
# Reference: Liu et al. (2021). Machine learning application for groundwater quality assessment:
...
csv_path = Path(__file__).parent / 'a_data_for_Programing_(ions_2000_for_dendrogram).csv'
...
pars = ['Label', 'TDS', 'pH', 'Ca', 'Mg', 'K', 'Na', 'KNa', 'Cl', 'SO4', 'HCO3', 'NO3']
df = raw_df[pars].copy()
...
# CBE_anions = df['Cl'] * 1 / Cl + df['SO4'] * 2 / SO4 + df['HCO3'] * 1 / HCO3 + df['NO3'] * 1 / NO3 
...
# tmp_df = df[(df['CBE'] <= 10) | (df['CBE'].isnull())][
#     ['ID', 'ShortID', 'SY', 'SM', 'pH', 'Ca', 'Mg', 'KNa', 'Cl', 'SO4', 'HCO3', 'NO3', 'TDS']].dropna(axis=0,
#                                                                                                            how='any')
...
# Print the statistical information of the remaining samples
df[['Label', 'TDS', 'pH', 'Ca', 'Mg', 'K', 'Na', 'KNa', 'Cl', 'SO4', 'HCO3', 'NO3']].describe()

# Data transformation
X_df = np.log(df[['TDS', 'pH', 'Ca', 'Mg', 'KNa', 'Cl', 'SO4', 'HCO3', 'NO3']])
...
Y_df = df[['Label', 'TDS', 'pH', 'Ca', 'Mg', 'K', 'Na', 'KNa', 'Cl', 'SO4', 'HCO3', 'NO3']]

# Clustering_the_geochemical_data_by_using_HCA
...
H = hierarchy.linkage(rescaledX, method='ward', metric='euclidean')
...
# Plot the dendrogram
hierarchy.dendrogram(H, leaf_rotation=90, leaf_font_size=0,
...
n_clusters = np.max(np.unique(hierarchy.fcluster(H, max_d, criterion='distance')))

# Calculate the number of samples for each cluster
cluster_size = [np.sum(hierarchy.fcluster(H, max_d, criterion='distance') == i + 1) for i in range(n_clusters)]
...
cum_cluster_size = np.cumsum(cluster_size) * 10
for i in range(n_clusters - 1):
    ax2.plot([cum_cluster_size[i], cum_cluster_size[i]], [1.5, 2.0], linestyle='--', color='k', lw=1.5)
...
ax2.text(0 + (cum_cluster_size[0] - 0) / 2, 1.5, 'C1 \nN=%d' % cluster_size[0], ha='center', va='bottom', fontsize=12)
ax2.text(cum_cluster_size[0] + (cum_cluster_size[1] - cum_cluster_size[0]) / 2, 1.5, 'C2 \nN=%d' % cluster_size[1], ha='center', va='bottom', fontsize=12)
# ax2.text(cum_cluster_size[1] + (cum_cluster_size[2] - cum_cluster_size[1]) / 2, 1.5, 'C3 \nN=%d' % cluster_size[2], ha='center', va='bottom', fontsize=12)
# ax2.text(cum_cluster_size[2] + (cum_cluster_size[3] - cum_cluster_size[2]) / 2, 1.5, 'C4 \nN=%d' % cluster_size[3], ha='center', va='bottom', fontsize=12)

plt.show()

# # Save the clustering results
# Y_df['SCI'] = hierarchy.fcluster(H, max_d, criterion='distance')
...
# # Draw the diagram
# gibbs.plot(format_df, unit='mg/L', figname='Gibbs diagram', figformat='jpg')
# plt.show()

# from wqchartpy import gaillardet

# # Draw the diagram
# gaillardet.plot(format_df, unit='mg/L', figname='Gaillardet diagram', figformat='jpg')
# plt.show()

# # Trilinear Piper plot created. Saving it to
# # C:\Users\Jing\Desktop\WQChartPy_revision\code_revision\WQChartPy\examples
